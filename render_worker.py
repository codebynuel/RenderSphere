...
def stream_blender_process(process, start_frame, end_frame, frame_step, samples, is_animation, progress_callback=None, should_cancel=None):
...
def normalize_render_input(job_input):
...
def render_blender_job(job_input, job_id=None, progress_callback=None, should_cancel=None):
    settings = normalize_render_input(job_input)
    ...
    try:
        ...
        print(
            ...
            f"frame_step={settings['frame_step']} "
            f"frame_count={settings['frame_count']} "
            f"animation={settings['is_animation']} "
            f"format={settings['output_format']} "
            f"resolution_pct={settings['resolution_pct']} "
            f"denoiser={settings['denoiser']} "
            f"scene={settings['scene_name'] or 'active'} "
            f"camera={settings['camera'] or 'scene camera'} "
            f"advanced_mode={settings['advanced_mode']} "
            f"gpu_device_type={settings['gpu_device_type']} "
            f"force_cpu={settings['force_cpu']} "
            f"allow_cpu_fallback={settings['allow_cpu_fallback']} "
            f"timeout_seconds={RENDER_TIMEOUT_SECONDS}"
        )

        print(f"Downloading source blend from R2 key {settings['file_key']}...")
        storage_client.download_file(bucket_name, settings['file_key'], local_blend_path)
        print(f"Starting headless {settings['engine']} render...")

        gpu_script = build_blender_setup_script(settings['render_settings'])
        ...
        if settings['scene_name']:
            render_command.extend(['-S', settings['scene_name']])
        ...
        if settings['is_animation']:
            render_command.extend(['-s', str(settings['start_frame']), '-e', str(settings['end_frame']), '-j', str(settings['frame_step']), '-a'])
        else:
            render_command.extend(['-f', str(settings['start_frame'])])
        ...
        stream_blender_process(
            process,
            settings['start_frame'],
            settings['end_frame'],
            settings['frame_step'],
            settings['samples'],
            settings['is_animation'],
        ...
        if settings['is_animation']:
            ...
            result_key = f"finished_renders/{job_id or int(time.time())}.zip"
        else:
            upload_file = find_rendered_frame(output_dir, settings['start_frame'], settings['output_extension'])
            result_key = f"finished_renders/{job_id or int(time.time())}.{settings['output_extension']}"

        ensure_tmp_space('upload')
        print(f"Uploading render output to R2 key {result_key}...")
        storage_client.upload_file(upload_file, bucket_name, result_key)
